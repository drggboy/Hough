...
def slider_binary(img_src):
    ...
    cv2.createTrackbar("thresh", "thresh_binary", 157, 255, trackChaned)
    while (True):
        thresh = cv2.getTrackbarPos("thresh", "thresh_binary")
        thresh, img_thresh = cv2.threshold(img_gray_show, thresh, 255, cv2.THRESH_BINARY)
...
if __name__ == '__main__':
    ...
    # 圈定感兴趣区域
    # img_roi = qd.img_qd(img_src,tune = True)
    img_roi = qd.img_qd(img_src, a=[68, 315], b=[1732, 937])
    # img_roi = qd.img_Sub(img_roi, a=[72, 557], b=[285, 988])
    ...
    # 获得车道线图像
    img_obj = hsv_filter(img_roi, tune=False)
    # img_obj = hsv_filter(img_roi, tune=True)
    ...
    # 高斯去噪
    # img_open = cv2.GaussianBlur(img_obj, (5, 5), 0)
    # img_show('img_open',img_open)

    # 横向闭运算
    kernel_d = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (61, 11))
    im_hclose = cv2.morphologyEx(img_open,cv2.MORPH_CLOSE,kernel_d,iterations=1)
    img_show('im_close', im_hclose)

    # 纵向闭运算
    kernel_z = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (2,41))
    ...
    thresh,roi_binary = cv2.threshold(roi_gray,157,255,cv2.THRESH_BINARY)
    # 通过滑块选取阈值
    # roi_binary = slider_binary(im_zclose)
    # img_show('roi_binary', roi_binary)

    # 检测角点，将角点处标红
    # dst = cv2.cornerHarris(roi_binary, 8, 31, 0.06)
    dst = cv2.cornerHarris(roi_binary, 21, 3, 0.06)
    corner_img = img_src.copy()
    corner_img[dst > 0.1 * dst.max()] = [0, 0, 255]
    img_show("corner_img",corner_img)
    # 取得角点图
    # img_black_white = np.zeros_like(img_src)
    # img_black_white[dst > 0.1 * dst.max()] = [255, 255, 255]
    # img_show('img_black_white',img_black_white)
    # np.save('.\data\jiao_dian.npy', img_black_white)
    ...
    lines = cv2.HoughLines(img_edges, 1, np.pi / 180, 75)
    # 转化为n行2列的 ndarray数组
